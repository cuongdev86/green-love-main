...
class AdminAudioController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($status) {
            if ($status == AudioEnum::PenddingTemp) {
                $query->where('status', '=', AudioEnum::Pendding);
            } else {
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = [
                ...
                'status' => AudioEnum::Pendding,
                'category_id' => $request->category_id,
                'user_id' => 1
            ];
            $upload = uploadFile($request, 'file', 'audios');
            if (!empty($upload)) {
    ...
    public function edit(string $id, string $slug)
    {
        $categories = Category::get();
        $audio = Audio::find($id);
        return view('admin.audio.edit', compact('categories', 'audio'));
    }
    ...
    public function update(Request $request, string $id)
    {
        try {
            DB::beginTransaction();
            $audio = Audio::find($id);
            if ($audio->status != AudioEnum::Pendding) {
                toastr()->warning('Bạn không thể cập nhật khi âm thanh này đang hoạt đông, hãy liên hệ admin');
                return redirect()->back();
            }
            $data = [
                'name' => $request->name,
            ...
            if (!empty($upload)) {
                ...
                $data['file_name'] = $upload['file_name'];
                if ($audio->file_path) {
                    $delete = deleteFile($audio->file_path);
                }
            }
            Audio::find($id)->update($data);
            DB::commit();
